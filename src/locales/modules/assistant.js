...
export default {
  ...
  quickProviderTitle: _('快速配置模型服务', 'Quick Model Setup', '快速設定模型服務'),
  quickProviderDesc: _('选择晴辰云免费测试，或使用词元 API 赞助服务；两套配置相互独立。', 'Choose QingChen Cloud for free testing or the sponsored CiyAPI service. Their configurations remain independent.', '選擇晴辰雲免費測試，或使用詞元 API 贊助服務；兩套設定相互獨立。'),
  quickEnterKeyFirst: _('填写 API Key 后获取模型', 'Enter an API Key to fetch models', '填寫 API Key 後取得模型'),
  qtcoolName: _('晴辰云', 'QingChen Cloud', '晴辰雲'),
  qtcoolDesc: _('ClawPanel 配套的免费签到测试平台，适合体验和功能验证', 'A free check-in testing platform for ClawPanel trials and functional verification', 'ClawPanel 配套的免費簽到測試平台，適合體驗和功能驗證'),
  qtcoolRecommend: _('免费测试', 'Free Testing', '免費測試'),
  qtcoolCheckin: _('签到 / 获取密钥', 'Check in / Get API Key', '簽到 / 取得金鑰'),
  qtcoolLearnMore: _('访问晴辰云 →', 'Visit QingChen Cloud →', '前往晴辰雲 →'),
  qtcoolKeyPlaceholder: _('粘贴晴辰云 API Key', 'Paste QingChen Cloud API Key', '貼上晴辰雲 API Key'),
  qtcoolInstructions: _('ClawPanel 不内置或自动读取晴辰云密钥，请自行注册 / 签到获取后手动粘贴', 'ClawPanel does not include or auto-read a QingChen Cloud key. Sign up or check in, then paste your own key.', 'ClawPanel 不內建或自動讀取晴辰雲金鑰，請自行註冊 / 簽到取得後手動貼上'),
  qtcoolFetchModels: _('获取模型', 'Fetch Models', '取得模型'),
  qtcoolFetching: _('获取中...', 'Fetching...', '取得中...'),
  qtcoolEnterKey: _('请输入晴辰云 API Key', 'Please enter a QingChen Cloud API Key', '請輸入晴辰雲 API Key'),
  qtcoolFillFirst: _('请先填写晴辰云 API Key 并选择模型', 'Enter a QingChen Cloud API Key and select a model first', '請先填寫晴辰雲 API Key 並選擇模型'),
  ...
  qtcoolSyncDesc: _('将当前晴辰云配置同步到 OpenClaw 全局配置', 'Sync the current QingChen Cloud config to OpenClaw', '將目前晴辰雲設定同步到 OpenClaw 全域設定'),
  qtcoolSyncTo: _('同步到 OpenClaw', 'Sync to OpenClaw'),
  qtcoolSyncToDesc: _('将晴辰云模型 {model} 写入 OpenClaw 并设为主模型', 'Write QingChen Cloud model {model} to OpenClaw and set it as primary', '將晴辰雲模型 {model} 寫入 OpenClaw 並設為主模型'),
  qtcoolSyncToTitle: _('确认同步到 OpenClaw', 'Confirm sync to OpenClaw', '確認同步到 OpenClaw'),
  qtcoolConfirmSync: _('确认同步', 'Confirm Sync', '確認同步'),
  ...
  ciyapiName: _('词元 API', 'CiyAPI', '詞元 API'),
  ciyapiDesc: _('支持 GPT、Claude 等前沿模型；充值 ¥1 到账 $1 平台额度，部分线路享折扣', 'GPT, Claude, and other frontier models; ¥1 funds $1 in usage credit, with discounts on selected routes', '支援 GPT、Claude 等前沿模型；儲值 ¥1 到帳 $1 平台額度，部分線路享折扣'),
  ciyapiRecommend: _('赞助推广', 'Sponsored', '贊助推廣'),
  ciyapiCheckin: _('注册 / 获取密钥', 'Sign up / Get API Key', '註冊 / 取得金鑰'),
  ciyapiLearnMore: _('模型与价格 →', 'Models & pricing →', '模型與價格 →'),
  ciyapiKeyPlaceholder: _('粘贴词元 API Key', 'Paste CiyAPI API Key', '貼上詞元 API Key'),
  ciyapiInstructions: _('第三方赞助推广，具体服务规则以词元 API 页面为准', 'Third-party sponsored placement; current CiyAPI terms apply', '第三方贊助推廣，具體服務規則以詞元 API 頁面為準'),
  ciyapiFetchModels: _('获取模型', 'Fetch Models', '取得模型'),
  ciyapiFetching: _('获取中...', 'Fetching...', '取得中...'),
  ciyapiEnterKey: _('请输入词元 API Key', 'Please enter a CiyAPI API Key', '請輸入詞元 API Key'),
  ciyapiFillFirst: _('请先填写词元 API Key 并选择模型', 'Enter a CiyAPI API Key and select a model first', '請先填寫詞元 API Key 並選擇模型'),
  ciyapiConnecting: _('连接中...', 'Connecting...', '連線中...'),
  ciyapiLoadingModels: _('正在获取模型列表...', 'Fetching model list...', '正在取得模型列表...'),
  ciyapiConnectFail: _('连接失败', 'Connection failed', '連線失敗'),
  ciyapiSelectModel: _('选择模型', 'Select model', '選擇模型'),
  ciyapiApply: _('一键配置', 'One-Click Config', '一鍵設定'),
  ciyapiConfigured: _('已配置词元 API', 'CiyAPI configured', '已設定詞元 API'),
  ciyapiAssistantOnly: _('仅配置助手', 'Configure assistant only', '僅設定助手'),
  ciyapiSetMain: _('设为主模型', 'Set as primary', '設為主模型'),
  ciyapiSetMainDone: _('已设为主模型', 'Set as primary model', '已設為主模型'),
  ciyapiMainSwitched: _('主模型已切换为 {model}', 'Primary model switched to {model}', '主模型已切換為 {model}'),
  ciyapiTestPass: _('连接测试通过', 'Connection test passed', '連線測試通過'),
  ciyapiTestFail: _('连接测试失败', 'Connection test failed', '連線測試失敗'),
  ciyapiSyncTitle: _('同步到 OpenClaw', 'Sync to OpenClaw'),
  ciyapiSyncDesc: _('将当前词元 API 配置同步到 OpenClaw 全局配置', 'Sync the current CiyAPI config to OpenClaw', '將目前詞元 API 設定同步到 OpenClaw 全域設定'),
  ciyapiSyncTo: _('同步到 OpenClaw', 'Sync to OpenClaw'),
  ciyapiSyncToDesc: _('将词元 API 模型 {model} 写入 OpenClaw 并设为主模型', 'Write CiyAPI model {model} to OpenClaw and set it as primary', '將詞元 API 模型 {model} 寫入 OpenClaw 並設為主模型'),
  ciyapiSyncToTitle: _('确认同步到 OpenClaw', 'Confirm sync to OpenClaw', '確認同步到 OpenClaw'),
  ciyapiConfirmSync: _('确认同步', 'Confirm Sync', '確認同步'),
  ciyapiSyncToDone: _('已同步到 OpenClaw', 'Synced to OpenClaw'),
  ciyapiSyncFail: _('同步失败', 'Sync failed', '同步失敗'),
  ciyapiWriteFail: _('写入配置失败', 'Failed to write config', '写入設定失敗'),
  ciyapiGatewayFail: _('Gateway 重启失败', 'Gateway restart failed', 'Gateway 重啟失敗'),
  ciyapiAllDone: _('全部完成', 'All done'),
  ciyapiSyncFrom: _('从 OpenClaw 读取', 'Read from OpenClaw', '從 OpenClaw 讀取'),
  ciyapiSyncFromTitle: _('确认从 OpenClaw 读取', 'Confirm read from OpenClaw', '確認從 OpenClaw 讀取'),
  ciyapiSyncFromDesc: _('将从 OpenClaw 读取配置：\nBase URL: {baseUrl}\nAPI Key: {apiKey}\n模型: {model}', 'Will read config from OpenClaw:\nBase URL: {baseUrl}\nAPI Key: {apiKey}\nModel: {model}', '將從 OpenClaw 讀取設定：\nBase URL: {baseUrl}\nAPI Key: {apiKey}\n模型: {model}'),
  ciyapiConfirmRead: _('确认读取', 'Confirm Read', '確認讀取'),
  ciyapiSyncFromDone: _('已从 OpenClaw 读取配置', 'Read config from OpenClaw', '已從 OpenClaw 讀取設定'),
  ciyapiReadFail: _('读取失败', 'Read failed', '讀取失敗'),
  ciyapiNoProvider: _('OpenClaw 中没有找到词元 API 配置', 'CiyAPI config not found in OpenClaw', 'OpenClaw 中沒有找到詞元 API 設定'),
