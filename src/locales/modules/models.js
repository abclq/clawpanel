...
export default {
  ...
  qtcoolDesc: _('ClawPanel 配套的免费签到测试平台，签到领取测试额度，适合体验和功能验证。', 'A free check-in testing platform for ClawPanel, ideal for trials and functional verification.', 'ClawPanel 配套的免費簽到測試平台，簽到領取測試額度，適合體驗和功能驗證。'),
  qtcoolMore: _('访问晴辰云 →', 'Visit QingChen Cloud →', '前往晴辰雲 →'),
  ...
  qtcoolNoKey: _('ClawPanel 不内置晴辰云密钥，请自行注册 / 签到获取 API Key 后粘贴。', 'ClawPanel does not include a QingChen Cloud key. Sign up or check in, then paste your own API Key.', 'ClawPanel 不內建晴辰雲金鑰，請自行註冊 / 簽到取得 API Key 後貼上。'),
  qtcoolCheckinPage: _('签到', 'Check-in', '簽到'),
  qtcoolDashboard: _('用户门户', 'User Portal', '使用者入口'),
  qtcoolDocs: _('接入文档', 'API Docs', '接入文件'),
  qtcoolSelectTitle: _('选择要添加的模型', 'Select models to add', '選擇要新增的模型'),
  qtcoolSelectHint: _('从晴辰云获取到 {count} 个可用模型，勾选需要的模型后点击添加。', 'Fetched {count} available models from QingChen Cloud. Select and add.', '從晴辰雲取得 {count} 個可用模型，勾選需要的模型後點擊新增。'),
  qtcoolKeyLabel: _('API Key', 'API Key'),
  qtcoolKeyCheckinLink: _('签到获取测试密钥 →', 'Get a testing key →', '簽到取得測試金鑰 →'),
  qtcoolKeyPlaceholder2: _('粘贴你的 API Key', 'Paste your API Key', '貼上你的 API Key'),
  qtcoolNoKeyWarn: _('请输入晴辰云 API Key', 'Please enter a QingChen Cloud API Key', '請輸入晴辰雲 API Key'),
  ...
  ciyapiDesc: _('支持 GPT、Claude 等主流前沿模型；充值 ¥1 到账 $1 平台额度，部分线路按折扣计费。', 'GPT, Claude, and other frontier models; ¥1 funds $1 in platform usage credit, with discounts on selected routes.', '支援 GPT、Claude 等主流前沿模型；儲值 ¥1 到帳 $1 平台額度，部分線路按折扣計費。'),
  ciyapiMore: _('查看模型价格 →', 'View model pricing →', '查看模型價格 →'),
  ciyapiCheckin: _('注册 / 获取密钥', 'Sign up / Get API Key', '註冊 / 取得金鑰'),
  ciyapiKeyPlaceholder: _('粘贴词元 API Key', 'Paste CiyAPI Key', '貼上詞元 API Key'),
  ciyapiFetchModels: _('获取模型列表', 'Fetch Model List', '取得模型列表'),
  ciyapiFetching: _('获取中...', 'Fetching...', '取得中...'),
  ciyapiNoKey: _('第三方赞助推广，服务规则以词元 API 页面为准。', 'Third-party sponsored placement; current CiyAPI terms apply.', '第三方贊助推廣，服務規則以詞元 API 頁面為準。'),
  ciyapiSignup: _('注册', 'Sign up', '註冊'),
  ciyapiDashboard: _('API 密钥', 'API Keys', 'API 金鑰'),
  ciyapiPricing: _('模型价格', 'Model Pricing', '模型價格'),
  ciyapiWallet: _('充值', 'Top up', '儲值'),
  ciyapiSelectTitle: _('选择要添加的模型', 'Select models to add', '選擇要新增的模型'),
  ciyapiSelectHint: _('从词元 API 获取到 {count} 个可用模型，勾选需要的模型后点击添加。', 'Fetched {count} available models from CiyAPI. Select and add.', '從詞元 API 取得 {count} 個可用模型，勾選需要的模型後點擊新增。'),
  ciyapiKeyLabel: _('API Key', 'API Key'),
  ciyapiKeyCheckinLink: _('获取 API Key →', 'Get API Key →', '取得 API Key →'),
  ciyapiKeyPlaceholder2: _('粘贴你的 API Key', 'Paste your API Key', '貼上你的 API Key'),
  ciyapiNoKeyWarn: _('请输入词元 API Key', 'Please enter a CiyAPI Key', '請輸入詞元 API Key'),
  ciyapiAddSelected: _('添加选中模型', 'Add Selected Models', '新增選中模型'),
  ciyapiNoneSelected: _('未选择任何模型', 'No models selected', '未選擇任何模型'),
  ciyapiAllExist: _('所选模型均已存在', 'All selected models already exist', '所選模型均已存在'),
  ciyapiAdded: _('已添加 {count} 个模型', 'Added {count} models', '已新增 {count} 個模型'),
  ciyapiProviderAdded: _('已添加词元 API（{count} 个模型）', 'Added CiyAPI ({count} models)', '已新增詞元 API（{count} 個模型）'),
  already: _('已有', 'Exists'),
